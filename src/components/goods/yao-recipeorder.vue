...
export default {
    ...
    methods: {
        ...
        submitOrder () {  // 提交
            ...
            var obj = { data: this.recipedata.docsuggestdrug, uid: this.uid, hid: self.recipedata.hid, rid: self.recipedata.id, did:self.recipedata.did, type: type, state: type, addid: this.userSite.id  }
            console.log(obj)
            self.$http.post('/mobile/wxorder/doc_drug_idea', obj).then(res => {
                console.log(res)
                if (res.code == 1) {
                    self.wxsjk(res.data)
